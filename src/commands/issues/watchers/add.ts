...
export default class Watch extends BaseCommand {
    ...
    static examples = [
        `$ jiraserver issues:votes:vote -a "MyAlias" --issue "theIssueKeyOrId" --user "theUsername" --json`,
        `$ jiraserver issues:votes:vote -a "MyAlias" --issue "theIssueKeyOrId" --user "theUsername" --csv`,
        `$ jiraserver issues:votes:vote -a "MyAlias" --issue "theIssueKeyOrId" --user "theUsername"`,
    ];
    static flags = {
        ...BaseCommand.flags,
    ...
    async run(): Promise<JiraCLIResponse<any>> {
        ...
        try {
            ...
            response.status = 0;
            response.message = 'Issue watched successffully';
