    ...
    async run(): Promise<JiraCLIResponse<ApplicationRole>> {
        ...
        try {
            const result = await connector.applicationRoles.update(this.flags.role, {
                key: this.flags.role,
                groups: this.flags.groups && this.flags.groups.length > 0 ? this.flags.groups : undefined,
                defaultGroups: this.flags['default-groups'] && this.flags['default-groups'].length > 0 ? this.flags['default-groups'] : undefined,
            }, this.flags.match);
